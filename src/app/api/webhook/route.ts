...
export async function POST(req: NextRequest) {
 ...
 try {
  if (payload !== null) {
   ...
   return NextResponse.json('Webhook Error: Payload is null', { status: 400 });
  }
 } catch (err: unknown) {
 ...
 if (event.type === 'checkout.session.completed') {
  ...
  // await transporter.sendMail(emailOptions as SendMailOptions);
 } else if (event.type === 'checkout.session.async_payment_failed') {
  console.log(`❌ Checkout session cancelled for session`, JSON.stringify(event, null, 2))
  const session = event.data.object as Stripe.Checkout.Session & { last_payment_error?: { message?: string } | undefined }
...
async function updateBookingStatus(session: Stripe.Checkout.Session & { billing_details?: { email?: string } }) {
  ...
  if (isUserPresent) {
   await db.user.update({
    ...
    data: {
     bookings: {
      connect: {
       id: parseInt(session.metadata?.bookingId ?? '0'),
      }
     }
    },
   })
...
async function createTransactionHistory(session: Stripe.Checkout.Session & { last_payment_error?: { message?: string } | undefined, billing_details?: { email?: string } }) {
 if (session.billing_details?.email) {
 ...
 await db.booking.update({
  ...
  data: {
   ...
   transactionHistory: {
    create: [{
     stripeTransactionId: session?.payment_intent?.toString() ?? '',
     failureReason: session.last_payment_error?.message ?? 'Unknown reason',
     amount: session.amount_total ?? 0,
     currency: session.currency ?? 'inr',
